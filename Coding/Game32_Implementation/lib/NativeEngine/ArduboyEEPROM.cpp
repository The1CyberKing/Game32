...
void EEPROMClass::commit() {
    // Save current game EEPROM buffer to SD card save file
    // Note: Arduboy2ESP::getGameName() returns the active game name
    extern const char* getActiveGameName();
    const char* gName = getActiveGameName();
    if (gName && gName[0] != '\0') {
        commitToFile(gName);
    }
}
